...
export default function RegistrationButton(props: RegistrationButtonProps) {
    ...
    const handleClick = async () => {
        ...
        await props.updateVolunteers();
        alert(`You have volunteered for ${props.event.subject}`);
        setIsLoading(false);
    }
    ...
    const hasSpace = props.event[`${config.EventCustomFieldSetName}.vacancy`] ? props.volunteers >= props.event[`${config.EventCustomFieldSetName}.vacancy`] : false

    // If the user already registrered
    // if (props.volunteers.map(v => v["contact.email_primary.email"]))

    return <button className="text-white font-semibold bg-secondary rounded-md w-full py-[6px] px-2 mb-2 disabled:bg-primary" onClick={handleClick} disabled={!(!registered) || !withinRegistration || !hasSpace}>
